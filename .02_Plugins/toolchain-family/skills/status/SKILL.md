...
permissions:
  ...
  forbidden: [Edit, Write, Agent, Skill, WebFetch, WebSearch, NotebookEdit, LSP, TaskCreate, TaskUpdate]
  bash_scope: "git status, bd list, ao ratchet — read-only"
  model_tier: micro
context:
  window: inherit
  intent:
...
|---------|-------|----------|
| Shows "BD_UNAVAILABLE" or "AO_UNAVAILABLE" | CLI tools not installed or not in PATH | Install missing tools via `go install` or download from the AgentOps releases page. Skill gracefully degrades by showing available state only. |
...
- [`recover`](../recover/SKILL.md) — Post-compaction context recovery (deeper state reconstruction)
- [`handoff`](../handoff/SKILL.md) — Structured session end with continuation context
