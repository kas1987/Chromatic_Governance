...
context:
  ...
  intel_scope: topic
permissions:
  allowed: [Read, Glob, Grep, Bash, Write, Agent, Skill]
  forbidden: [Edit, WebFetch, WebSearch, NotebookEdit, LSP, TaskCreate, TaskUpdate]
  bash_scope: "bd create/deps, ao search/ratchet, git log/status, jq, grep, wc, find"
  write_scope: ".agents/plans/ only"
  model_tier: medium
  note: "Agent: Explore subagent only (read-only codebase research) — no general-purpose workers"
---
...
| Allowed | Forbidden |
|---------|-----------|
| Read, Glob, Grep, Bash, Write, Agent† | Edit, Skill, WebFetch, WebSearch, NotebookEdit, LSP, TaskCreate, TaskUpdate |

**Bash:** bd create/deps, ao search/ratchet, git log/status, jq, grep, wc, find
**Write:** .agents/plans/ only

†Agent: Explore subagent only (read-only codebase research). No general-purpose workers.
...
**CLI dependencies:** bd (issue creation). `bd` is ALWAYS used for tracking — never TaskCreate, TaskUpdate, or TaskList. If bd is truly unavailable, write the plan to `.agents/plans/` as markdown only and surface an error.
...
### Step 0.5: Load Harness Context (Standalone Guard)

When invoked directly (not via `/discovery`), load harness context if available:
```bash
HARNESS_CTX_FILE="${HOME}/.claude/.agents/rpi/harness-context.json"
if [ -f "$HARNESS_CTX_FILE" ]; then
  HARNESS_CTX=$(cat "$HARNESS_CTX_FILE" 2>/dev/null)
  echo "Harness context loaded: $(echo "$HARNESS_CTX" | jq -r '.loaded_at // "unknown"' 2>/dev/null)"
fi
```
Use `global_goals`, `constraints`, and `topology` from harness context to validate plan alignment.
...
3. **Inline code blocks** — for non-obvious constructs (struct definitions, CLI flags, config snippets). Verify all inline Python snippets are importable before including them in issue descriptions — workers copy them verbatim. For Python projects use `python -c "import <module>"` not `go build ./...`:
   ```go
...
1. **Unit tests**: `ruff check . && pytest tests/ -k "rate_limit" -v --tb=short`
2. **Import check**: `python -c "from 01_INTAKE import github_notification_router"`
3. **Manual simulation**:
   ```bash
   # Start server
   go run ./cmd/server/ &
...
- **Validation commands (`metadata.validation`):** Include runnable checks (e.g., `ruff check . && pytest --tb=short`, `bash -n script.sh`). Workers run these before reporting done.
...
| "Tests pass" | `tests: "pytest --tb=short"` |
...
| Issue 1 | tests | `pytest tests/test_auth.py -v --tb=short` |
| Issue 2 | files_exist | `["docs/api-v2.md"]` |

## Verification

1. **Unit tests**: `pytest tests/test_<module>.py -v --tb=short`
2. **Full suite**: `ruff check . && pytest --tb=short`
3. **Manual simulation**:
   ```bash
...
### Step 7: Create Issues in Beads

**ALWAYS use `bd` for tracking — never TaskCreate, TaskUpdate, or TaskList.**

Create beads issues to enable progress tracking across sessions:
```bash
# Create epic first
...
### Step 8: Log Plan Finalization (Gate 2)

**Default (autonomous) mode:** Log the result and proceed immediately to Step 9.

```
Plan finalized: N tasks, M waves. Proceeding (use --review to gate).
```

**Only use AskUserQuestion if `--review` flag is explicitly passed:**
...
**Do not wait for approval unless `--review` is set.**
...
printf '%s\n' "$(jq -cn \
  --arg plan "${PLAN_PATH:-}" \
  --arg epic "${EPIC_ID:-}" \
  '{event:"plan_complete",plan_path:$plan,epic_id:$epic,timestamp:(now|todate)}')" \
