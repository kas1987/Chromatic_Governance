...
permissions:
  ...
  bash_scope: "git diff/log/status, jq, ls — read-only"
  write_scope: ".agents/council/ only — no source code, no learnings"
  model_tier: large
...
**Bash:** git diff/log/status, jq, ls — read-only
**Write:** .agents/council/ only — no source code, no learnings

*WebFetch/WebSearch: forbidden at council orchestrator level. EVIDENCE_JUDGE subagents carry their own WebFetch grant.
...
```bash
ao ratchet record council 2>/dev/null || true
```

**Output completion marker** (matches consensus verdict):
```
<promise>PASS</promise>
<promise>WARN</promise>
