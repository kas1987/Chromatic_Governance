...
permissions:
  allowed: [Read, Glob, Grep, Bash, Write, Agent]
  forbidden: [Edit, WebFetch, WebSearch, NotebookEdit, LSP, Skill, TaskCreate]
  bash_scope: "git log/diff/blame, ao codex/ratchet, bd children/list/show/remember, jq, bash hooks/finding-compiler.sh"
  write_scope: ".agents/learnings/, .agents/findings/, .agents/council/, .agents/rpi/next-work.jsonl — no source code"
  model_tier: large
---
...
| Allowed | Forbidden |
|---------|-----------|
| Read, Glob, Grep, Bash, Write, Agent | Edit, WebFetch, WebSearch, NotebookEdit, LSP, Skill, TaskCreate |

**Bash:** git log/diff/blame, ao codex/ratchet, bd children/list/show/remember, jq, bash hooks/finding-compiler.sh
**Write:** .agents/learnings/, .agents/findings/, .agents/council/, .agents/rpi/next-work.jsonl — no source code
...
| `--skip-activate` | off | Extract and process learnings but skip Phase 4 `bd remember` promotions. |
...
# Standalone beads feeding (no Chronicle active)
# When post-mortem runs outside an RPI lifecycle, Chronicle never spawns to consume
# next-work.jsonl. Feed items directly via bd create so they reach the backlog immediately.
if command -v bd &>/dev/null && [ "${RPI_CHRONICLE_ACTIVE:-false}" != "true" ]; then
  while IFS= read -r item_b64; do
    ...
    bd create "$title" -t "$bd_type" -p "$bd_priority" -d "$desc" 2>/dev/null \
      && echo "Created bead: $title"
  done < <(echo "$VALID_ITEMS_JSON" | jq -r '.[] | @base64' 2>/dev/null)
fi
```
...
/post-mortem              <-- You are here
    ...
    |-- Phase 4: Activate (promote via `bd remember`, compile constraints)
