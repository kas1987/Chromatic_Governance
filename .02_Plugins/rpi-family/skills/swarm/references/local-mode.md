...
If running in Claude runtime, prefer declarative isolation via the `Agent` tool's `isolation` parameter:

```python
# Spawn worker with automatic isolated worktree
Agent(
    description="worker-<task-id>: implement <task-subject>",
    subagent_type="general-purpose",
    model="sonnet",
    isolation="worktree",        # <-- creates fresh worktree per agent
    run_in_background=True,
    prompt="..."
)
```

The runtime creates a temporary git worktree branch for each agent. The worktree is returned in the agent result as `worktreePath`. When the agent completes, check `worktreePath` and merge back:

```bash
# After worker completes: merge the isolated branch back
git merge --no-ff <worktreePath-branch> -m "chore: merge swarm/<task-id>"
git worktree remove --force <worktreePath>
```

**Validation step:** Before merging, verify:
```bash
test -n "$worktreePath" && test -d "$worktreePath" \
  && git -C "$worktreePath" rev-parse --is-inside-work-tree
```

Fallback to manual `git worktree` management (see `bin/swarm-worktree-setup.sh` and `bin/swarm-worktree-teardown.sh`) when:
- Running outside Claude Code runtime
- Declarative `isolation` parameter is unavailable
- Workers need pre-seeded file content before spawning

See concrete Agent call and merge-back pattern above.
...
> **Governance required.** Before spawning, read `skills/shared/references/subagent-governance.md`. Every worker must receive a role assignment and the mandatory governance header. No exceptions.

### Role → subagent_type Selection (MANDATORY)

Assign a role before spawning. The role controls which `subagent_type` to use:

| Task type | Role | subagent_type | Writes? |
|-----------|------|--------------|---------|
| Code implementation, bug fix | `IMPL_WORKER` | `general-purpose` | Yes — manifest only |
| Test writing | `TEST_WORKER` | `general-purpose` | Yes — test files only |
| Research, file exploration | `EXPLORER` | `Explore` | No (runtime-enforced) |
| Plan review, judgment | `JUDGE` | `Explore` | No (runtime-enforced) |
| Spec/contract writing | `SPEC_WRITER` | `general-purpose` | Yes — spec files only |

**Critical:** Do NOT use `general-purpose` for judges or explorers. `Explore` agents are read-only at the runtime level — using it for read-only roles is the only reliable tool restriction available.
...
| Explorers | haiku | Read-only search tasks — cheapest acceptable quality |

### Spawn Protocol

For each ready task:

1. **Assign role** — Determine worker role from task type (IMPL_WORKER, TEST_WORKER, etc.)
2. **Pre-assign** — Mark the task owned by `worker-<task-id>` before spawning (prevents race conditions)
3. **Spawn** — Create a parallel subagent with the worker prompt (see below)
4. **Track** — Map `worker-<task-id>` to agent handle for waits/retries/cleanup

All workers in a wave spawn in parallel. New team/agent-group per wave = fresh context (Ralph Wiggum preserved).

### Worker Prompt Template

Every worker receives this prompt (adapt to your runtime's spawn mechanism). The governance header MUST come first — do not reorder or omit it.
...
<file from metadata.files — one per line>
<file from metadata.files — one per line>
If no files listed: you have READ-ONLY access.

PERMITTED ACTIONS:
- Read, Glob, Grep any file in the repo for context
- Bash for build/test/lint commands ONLY
- Write/Edit ONLY the files listed in FILE SCOPE above

FORBIDDEN — DO NOT DO THESE:
- Spawn subagents (Agent tool) — escalate to lead instead
- Read credential/secret files: settings.json, .env, *.pem, *.key, *.token
- Run git add, git commit, git push — the lead commits
- Make external network calls (curl, wget, requests.get, fetch)
- Modify files outside your FILE SCOPE without lead approval

SCOPE VIOLATION PROTOCOL:
If you need a file NOT in FILE SCOPE: write blocked result and signal lead.
Do NOT touch the out-of-scope file under any circumstances.
...
Validation command: <metadata.validation.tests or metadata.validation.command>

Instructions:
1. Execute your pre-assigned task independently — create/edit files as needed
2. Run validation command to verify your work before reporting done
3. Write your result to .agents/swarm/results/<task-id>.json (see format below)
4. Send a SHORT completion signal to the lead (under 100 tokens)
5. If blocked, write blocked result to same path and signal the lead
...
> **Note:** Adjust the `Role:` line in the governance header to match the actual role (`IMPL_WORKER`, `TEST_WORKER`, `SPEC_WRITER`, etc.). For `EXPLORER`/`JUDGE` roles, use `subagent_type="Explore"` and omit the Edit/Write/Bash permissions block entirely — the governance header still appears but read-only is enforced at the runtime level.
