...
Judges are read-only reviewers — use `subagent_type="Explore"` (runtime-enforced read-only). See `skills/shared/references/subagent-governance.md` for the full role registry.
...
Use `subagent_type="Explore"` for read-only agents (JUDGE, EXPLORER). Use `"general-purpose"` for agents that need to write files (IMPL_WORKER, TEST_WORKER, SPEC_WRITER). Never use `"general-purpose"` for judges — they get write access and risk accidental modification.
