...
permissions:
  allowed: [Read, Glob, Grep, Bash, Write, Agent, TaskCreate, TaskUpdate, TaskList, TaskGet]
  forbidden: [Edit, WebFetch, WebSearch, NotebookEdit, LSP]
  bash_scope: "git log/commit/diff/add/status/worktree, bd list/update/create, ao context/lookup, jq, mkdir, rm"
  write_scope: ".agents/swarm/, wave checkpoint files — orchestrator does NOT edit source files"
  model_tier: medium
  note: "Edit is forbidden at orchestrator level — workers (subagents) edit source files"
---
...
| Allowed | Forbidden |
|---------|-----------|
| Read, Glob, Grep, Bash, Write, Agent, TaskCreate, TaskUpdate, TaskList, TaskGet | Edit†, WebFetch, WebSearch, NotebookEdit, LSP |

**Bash:** git log/commit/diff/add/status/worktree, bd list/update/create, ao context/lookup, jq, mkdir, rm
**Write:** .agents/swarm/, wave checkpoint files — swarm orchestrator does NOT edit source files

†Edit: workers (subagents) edit source, not the swarm orchestrator.
...
> **Prefer `/codex-team` for simple 2+ task dispatches.** `/swarm` is for dependency-heavy wave orchestration with tight shared-state coordination. For parallel work without dependencies between tasks, `/codex-team` is the canonical entry point — it auto-selects backend (Codex sub-agents, Codex CLI, OpenCode, then `/swarm`) and has built-in file-conflict prevention. Use `/swarm` only when you need wave dependency management, bd-driven multi-wave orchestration, or Claude-native team coordination.

**Integration modes:**
- **Direct** - Create bd list tasks, invoke `/swarm`
- **Via Crank** - `/crank` creates tasks from beads, invokes `/swarm` for each wave
- **From codex-team fallback** - `/codex-team` delegates here when Codex unavailable
...
Mayor (this session)
    |
    +-> Plan: bd create with dependencies
    |
    ...
    |
    +-> Assign: bd update(taskId, owner="worker-<id>", status="in_progress")
    |
...
Use bd list to see current tasks. If none, create them:

```
bd create(subject="Implement feature X", description="Full details...",
  metadata={"issue_type": "feature", "files": ["src/feature_x.py", "tests/test_feature_x.py"], "validation": {...}})
bd update(taskId="2", addBlockedBy=["1"])  # Add dependencies after creation
```

#### Task Typing + File Manifest

Every bd create **must** include `metadata.issue_type` plus a `metadata.files` array. `issue_type` drives active constraint applicability and validation policy; `files` enable mechanical conflict detection before spawning a wave.
...
- Preserve the same `metadata.issue_type` on bd update / TaskCompleted payloads so task-validation can apply active constraints without guessing.
...
This produces a 5-section briefing (GOALS, HISTORY, INTEL, TASK, PROTOCOL) at `.agents/rpi/briefing-current.md` with secrets redacted. Include the briefing path in each worker's bd create description so workers start with full project context.
...
2. **Inject manifests** back into tasks:
   ```
   bd update(taskId=task.id, metadata={"files": [explored_files]})
   ```

Once all tasks have manifests, proceed to Step 2 where the Pre-Spawn Conflict Check enforces file ownership.

### Step 1b: Assign Worker Roles (MANDATORY, before any spawn)

Before spawning, assign every task a worker role. Role drives `subagent_type` selection — the only runtime-enforced tool restriction available in Claude Code.

| Task type | Role | subagent_type | Key restriction |
|-----------|------|--------------|----------------|
| Code implementation / bug fix | `IMPL_WORKER` | `general-purpose` | File manifest only |
| Test writing | `TEST_WORKER` | `general-purpose` | Test file paths only |
| File/codebase exploration | `EXPLORER` | `Explore` | Read-only (runtime-enforced) |
| Plan or spec review | `JUDGE` | `Explore` | Read-only (runtime-enforced) |
| Contract/spec writing | `SPEC_WRITER` | `general-purpose` | Spec output paths only |

Full governance rules, Codex sandbox requirements, and the mandatory governance header template are in `skills/shared/references/subagent-governance.md`. Read that doc before the first spawn of any wave.
...
bd update(taskId="1", status="completed")

# Add dependency between tasks
bd update(taskId="2", addBlockedBy=["1"])
```
...
- **bd list as memory** - State persists in task status, not agent context
...
When `/crank` invokes `/swarm`: Crank bridges beads to bd list, swarm executes with fresh-context agents, crank syncs results back.
...
| Just swarm, no beads | `/swarm` directly | bd list only, skip beads |
...
2. **Create bd list tasks** from wave entries (one `bd create` per entry):
...
**What happens:**
1. Agent identifies unblocked tasks from bd list (e.g., "Create User model")
2. Agent selects spawn backend using runtime-native priority (Claude session -> Claude teams; Codex session -> Codex sub-agents)
3. Agent spawns worker for task #1, assigns ownership via bd update
...
**What happens:**
1. User creates bd list tasks with bd create
...
**Result:** Parallel execution of independent tasks using bd list only.
...
3. Agent creates bd list tasks from wave entries (priority-sorted)
...
- [references/subagent-governance.md](references/subagent-governance.md)
