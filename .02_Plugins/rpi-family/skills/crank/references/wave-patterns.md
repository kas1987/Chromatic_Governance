...
1.5. **FILE SCOPE violation check (FAIL-CLOSED, mandatory):**

   Collect every file path declared in `metadata.files` across all TaskCreate calls in this wave. Compare against the actual changed files from step 1:

   ```bash
   # DECLARED: union of all metadata.files arrays from this wave's TaskCreate calls
   # ACTUAL:   output of `git diff $WAVE_START_SHA HEAD --name-only`
   DECLARED_FILES=(<union of metadata.files from wave TaskCreates>)
   ACTUAL_FILES=$(git diff $WAVE_START_SHA HEAD --name-only)

   # Files changed but NOT in any worker's declared scope
   SCOPE_VIOLATIONS=$(comm -13 \
     <(printf '%s\n' "${DECLARED_FILES[@]}" | sort) \
     <(echo "$ACTUAL_FILES" | sort))

   if [[ -n "$SCOPE_VIOLATIONS" ]]; then
     echo "SCOPE VIOLATION: files changed outside declared FILE SCOPE:"
     echo "$SCOPE_VIOLATIONS"
     # FAIL the wave immediately — do NOT advance to next wave
     WAVE_VERDICT="FAIL"
     WAVE_VERDICT_REASON="Files modified outside declared manifest: $SCOPE_VIOLATIONS"
   fi
   ```

   **Why this matters:** Without this gate, workers can silently write thousands of LOC in unauthorized directories, break passing tests, and require multi-commit surgical revert (observed in chromatic-harness-v2 Wave 3, 2026-05-28 — 2183 unauthorized LOC across 12 files). This check costs one `comm` call and catches the violation before it lands in `master`.

   **On FAIL:** Do NOT advance to next wave. Revert unauthorized files (`git checkout $WAVE_START_SHA -- <file>` for each violation), then re-dispatch the worker with an explicit governance header: `"You MUST NOT write, create, or modify files outside FILE SCOPE: [list]. Violations detected by diff gate will cause automatic revert."`. Count as a RETRY for the Node Repair Operator.

   **Opt-out:** `--no-scope-check` flag skips this check (e.g., for exploratory scaffold waves where manifest is intentionally open).
