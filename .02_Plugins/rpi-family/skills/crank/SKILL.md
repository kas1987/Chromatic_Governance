...
permissions:
  allowed: [Read, Glob, Grep, Bash, Write, Skill, Agent, TaskCreate, TaskUpdate, TaskList, TaskGet]
  forbidden: [Edit, WebFetch, WebSearch, NotebookEdit, LSP]
  bash_scope: "git log/commit/diff/add/status, ao context/flywheel/forge/lookup, bd list/update/create, jq, mkdir, cat"
  write_scope: ".agents/crank/, .agents/rpi/phase-2-summary-*.md, .agents/chronicle/events.jsonl — orchestrator does NOT write source code"
  model_tier: large
...
| Allowed | Forbidden |
|---------|-----------|
| Read, Glob, Grep, Bash, Write, Skill, Agent, TaskCreate, TaskUpdate, TaskList, TaskGet | Edit†, WebFetch, WebSearch, NotebookEdit, LSP |

**Bash:** git log/commit/diff/add/status, ao context/flywheel/forge/lookup, bd list/update/create, jq, mkdir, cat
**Write:** .agents/crank/, .agents/rpi/phase-2-summary-*.md, .agents/chronicle/events.jsonl — orchestrator does NOT write source code

†Edit: workers (subagents) edit source code, not the crank orchestrator itself.
...
**Subagent governance (MANDATORY):** Before spawning workers via /swarm, read `skills/shared/references/subagent-governance.md` and assign every task a worker role. Role selection controls `subagent_type` (the only runtime-enforced tool restriction). IMPL_WORKER/TEST_WORKER tasks → `general-purpose`; EXPLORER/JUDGE tasks → `Explore` (read-only). Every worker prompt MUST include the governance header from that doc with FILE SCOPE populated from `metadata.files`. Empty FILE SCOPE = blocked spawn, not unconstrained workers.
...
Also append a chronicle event for the Chronicle Observer (if `.agents/chronicle/events.jsonl` exists):

```bash
if [ -f .agents/chronicle/events.jsonl ] || [ -d .agents/chronicle ]; then
  TASKS_JSON=$(echo "$COMPLETED_IDS" | tr ' ' '\n' | jq -R . | jq -s .)
  COMMITS_JSON=$(git log --oneline "${WAVE_START_SHA}..HEAD" \
    --format="%H" 2>/dev/null | jq -R . | jq -s . || echo "[]")
  printf '%s\n' "$(jq -cn \
    --arg epic "$EPIC_ID" \
    --argjson wave "$wave" \
    --argjson tasks "$TASKS_JSON" \
    --argjson commits "$COMMITS_JSON" \
    '{event:"wave_complete",epic_id:$epic,wave:$wave,tasks_done:$tasks,commits:$commits,status:"done",timestamp:(now|todate)}')" \
...
If hooks or `lib/hook-helpers.sh` were modified, verify embedded copies are in sync: `cd cli && # make sync-hooks — skip in Python repos (no Makefile)`.
...
```bash
if command -v ao &>/dev/null; then
  ao forge transcript 2>/dev/null || true
  ao flywheel close-loop --quiet 2>/dev/null || true
  ao metrics flywheel status 2>/dev/null || true
  ao pool list --status=pending 2>/dev/null || true
else
  echo "ao unavailable — run /post-mortem manually to extract learnings"
fi
```
