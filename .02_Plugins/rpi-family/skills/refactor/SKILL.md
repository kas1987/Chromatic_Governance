...
- [`complexity`](../complexity/SKILL.md) — Analyze code complexity metrics before refactoring
- [`standards`](../standards/SKILL.md) — Language-specific conventions
- [`vibe`](../vibe/SKILL.md) — Validate code quality post-refactor
- [`bug-hunt`](../bug-hunt/SKILL.md) — If refactoring uncovers bugs
- [`implement`](../implement/SKILL.md) — If refactoring requires new code

---

## Examples

### Extract a god function into smaller units
```
/refactor internal/processor.go --extract-functions
```
Identifies functions over complexity threshold, extracts logical units, verifies tests still pass.

### Rename symbols for clarity across a package
```
/refactor --rename pkg/api
```
Suggests clearer names for exported symbols based on usage patterns.

### Eliminate duplicated logic across multiple files
```
/refactor --dedup ./internal/
```
Finds repeated code blocks, extracts shared helpers, updates all call sites.

> Extended refactoring patterns and decision trees: [`references/support.md`](references/support.md)
...
| Problem | Cause | Fix |
|---------|-------|-----|
| Tests fail after refactor | Behavior accidentally changed | `git diff` the refactored function; compare pre/post logic carefully |
| Scope creep: diff is growing too large | Refactor is touching too many concerns | Split into smaller PRs; commit each extracted unit separately |
| Complexity doesn't drop after extraction | Extracted function is still complex | Run `/complexity` again; the hotspot moved, not resolved |
| Circular import introduced | New package arrangement creates cycle | Restructure: move shared types to a separate low-level package |
| Pre-commit lint fails on renamed symbols | Formatter or linter sees unresolved reference | Run `go build ./...` or `tsc --noEmit` to find all reference sites before committing |
