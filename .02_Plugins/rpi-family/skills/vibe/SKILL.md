...
permissions:
  ...
  bash_scope: "git diff/log/status, jq, ao ratchet/lookup, bd close/list, bash hooks/ (read-only)"
  write_scope: ".agents/council/YYYY-MM-DD-vibe-*.md only — no source code"
  model_tier: large
---
...
**Bash:** git diff/log/status, jq, ao ratchet/lookup, bd close/list, bash hooks/ (read-only hooks)
**Write:** .agents/council/YYYY-MM-DD-vibe-*.md only
