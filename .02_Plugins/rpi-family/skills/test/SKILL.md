...
- **Activate the project virtualenv before running pytest** — on Windows/Git Bash:
  ```bash
  source .venv/Scripts/activate 2>/dev/null || source .venv/bin/activate 2>/dev/null || true
  ```
  Without this, `pytest` resolves to system Python and misses all project dependencies.
- **FastAPI endpoints need `TestClient`** — import from `fastapi.testclient`, pass the `app` object, do NOT test routes with plain `requests`. Use `conftest.py` to expose a shared `client` fixture:
  ```python
  ...
  def client():
      with TestClient(app) as c:
          yield c
  ```
- **Run `ruff check .` before coverage** — formatting errors surface as test-collection failures. Wire it as a pre-coverage gate:
  ```bash
  ruff check . && pytest --cov --cov-report=term-missing
  ```
...
source .venv/Scripts/activate 2>/dev/null || source .venv/bin/activate 2>/dev/null || true
ruff check . 2>&1 | tee .agents/test/ruff-check.txt
pytest --cov --cov-report=term-missing --cov-report=json:.agents/test/coverage.json --tb=short 2>&1 | tee .agents/test/coverage-raw.txt
...
```
Analyzes coverage, writes table-driven tests, re-runs until green.

### TDD loop for a new feature
```
/test --mode=tdd
```
Red → green → refactor cycle with cycle log in `.agents/test/tdd-log.md`.

### Coverage gap analysis only
```
/test --mode=coverage --min-coverage=80
```
Keeps generating tests until the 80% threshold is met.

> Extended patterns and bad-test catalogue: [`references/support.md`](references/support.md)
...
| Problem | Cause | Fix |
|---------|-------|-----|
| Coverage command not found | Test framework not installed | Run language setup: `go install`, `pip install pytest-cov`, `npm i -D vitest` |
| Generated tests fail on first run | Source function behavior differs from expectation | Read the failure; update assertion to match actual (not expected) behavior, then document |
| TDD red step passes immediately | Feature already exists | Test must fail first — pick a behavior not yet implemented |
| Coverage drops after adding tests | New test file introduced a compile error | Run `go build ./...` or `tsc --noEmit` before running coverage |
| `.agents/test/` not created | Write permission or path issue | `mkdir -p .agents/test` and retry |

---

## See Also

- [`standards`](../standards/SKILL.md) — Language-specific test conventions (naming, patterns)
- [`complexity`](../complexity/SKILL.md) — Identify high-risk code to prioritize for coverage
