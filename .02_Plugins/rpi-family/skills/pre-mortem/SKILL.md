...
permissions:
  ...
  forbidden: [Edit, WebFetch, WebSearch, NotebookEdit, LSP, TaskCreate]
  bash_scope: "ao search/ratchet, bd update, jq, bash hooks/finding-compiler.sh, ls"
  write_scope: ".agents/council/, .agents/findings/, .agents/pre-mortem-checks/ — no source code"
  model_tier: large
  note: "WebFetch/WebSearch forbidden at orchestrator level — EVIDENCE_JUDGE subagents carry their own grant"
---
...
| Allowed | Forbidden |
|---------|-----------|
| Read, Glob, Grep, Bash, Write, Agent, Skill | Edit, WebFetch*, WebSearch*, NotebookEdit, LSP, TaskCreate |

**Bash:** ao search/ratchet, bd update, jq, bash hooks/finding-compiler.sh, ls
**Write:** .agents/council/, .agents/findings/, .agents/pre-mortem-checks/ — no source code

*WebFetch/WebSearch: forbidden at orchestrator level. EVIDENCE_JUDGE subagents may use WebFetch per their governance grant.
...
```
Surface `constraints` and `topology` to council judges as additional risk context.
...
**Enforcement gate (always runs, never blocked by `--quick`):**
```bash
CHECKS_DIR=".agents/pre-mortem-checks"
REGISTRY=".agents/findings/registry.jsonl"
CHECKS_FOUND=0
if ls "${CHECKS_DIR}"/*.md 2>/dev/null | head -1 | grep -q .; then
  CHECKS_FOUND=$(ls "${CHECKS_DIR}"/*.md 2>/dev/null | wc -l)
  echo "Pre-mortem checks loaded: ${CHECKS_FOUND} files from ${CHECKS_DIR}/"
elif [ -f "${REGISTRY}" ]; then
  echo "Compiled checks absent — falling back to registry: ${REGISTRY}"
else
  echo "No prevention context found — proceeding without compiled checks"
fi
```
...
printf '%s\n' "$(jq -cn \
  ...
  --arg epic "${EPIC_ID:-}" \
  '{event:"pre_mortem_complete",verdict:$verdict,epic_id:$epic,timestamp:(now|todate)}')" \
...
**Output completion marker** (matches verdict):
```
<promise>PASS</promise>   # no blocking risks
<promise>WARN</promise>   # risks identified, proceed with caution
<promise>FAIL</promise>   # blocking risks, re-plan required
```
