...
context:
  ...
  intel_scope: topic
permissions:
  allowed: [Read, Glob, Grep, Bash, Write, Edit, Skill, Agent]
  forbidden: [WebFetch, WebSearch, NotebookEdit, LSP, TaskCreate, TaskUpdate]
  bash_scope: "git diff/log/status/add/commit, go test/build/vet, npm test, pytest, ruff, bd update/show/ready, ao ratchet/lookup/context, jq"
  write_scope: "source files, test files — no .agents/ orchestration files"
  model_tier: medium
---
...
# Python (activate venv first on Windows):
...
> **chromatic-inbox-harness specific:** After any change to `01_INTAKE/`, `02_ROUTING/`, or `03_DB/`,
> verify audit side-effects by checking that `emit_*` calls in `04_OBSERVABILITY/audit_emitter.py`
> fire correctly. Run: `python -c "from src.audit_emitter import emit_intake_event; print('audit_emitter OK')"`.
> If touching NSQ consumers (prism-autonomy-harness), confirm queue contracts haven't changed.
