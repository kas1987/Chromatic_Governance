# Agent Handoff: Cartographer — Hermes Local-Worker Integration

> PDR-008 / HERMES-008. Adapted from the source packet
> `hermes_harness_pdr_package/12_HANDOFFS/agent_packets/cartographer_hermes_packet.md`
> to this repository's actual layout. The Cartographer **runtime** is registered by
...
Cartographer — mapping / topology specialty. Maps the Hermes local-worker integration:
which routing files, capability tiers, and governance gates exist, how they connect, and
where the additive Hermes patch lands relative to the live router. Produces maps as docs.
...
- `config/agents.yaml → cartographer` (`status: inactive`,
  `branch_prefix: agent/cartographer/`, `require_pull_request: true`,
...
Map the Hermes local-worker integration: enumerate the routing/capability/gate files,
describe how they connect, and confirm the additive Hermes patch's position relative to
the live router — without mutating routing behavior.
