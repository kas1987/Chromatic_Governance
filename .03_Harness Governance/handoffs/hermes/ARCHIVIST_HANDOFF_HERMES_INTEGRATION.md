# Agent Handoff: Archivist — Hermes Local-Worker Integration

> PDR-008 / HERMES-008. Adapted from the source packet
> `hermes_harness_pdr_package/12_HANDOFFS/agent_packets/archivist_hermes_packet.md`
> to this repository's actual layout. The Archivist **runtime** is registered by
...
Archivist — records / retention specialty. Curates the durable record of the Hermes
local-worker integration: eval receipts, supersession notes, and decision history are
complete, well-formed, and discoverable. Produces archive/index docs.
...
- `config/agents.yaml → archivist` (`status: inactive`,
  `branch_prefix: agent/archivist/`, `require_pull_request: true`,
...
Keep the durable record of the Hermes integration accurate: verify every mission's eval
receipt is present and schema-valid, that supersessions are recorded, and that decision
history is indexed — without altering routing behavior.
...
- [`schemas/eval_receipt.schema.json`](../../schemas/eval_receipt.schema.json) and
  ...
  — the receipt contract this agent curates (HERMES-005/006).
...
- Add tests.
- Add docs (archive indexes, retention notes).
