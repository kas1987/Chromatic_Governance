...
| HERMES-006 | P2 | ✅ done | Merge the two CI gate plans into one workflow | `.github/workflows/hermes-gate.yml` — done 2026-06-16: single gate keyed on `eval_receipt`, scoped to `.03_Harness Governance/**`. Four jobs: **schema-validate** (validates every `operations/receipts/*.eval_receipt.json` against the schema; rejects invalid; advisory-pass when none present), **receipt-required** (framework Phase 3, PR-only: fails a PR that changes `.03_Harness Governance/` mission files without adding/updating a receipt; `no-receipt` label waives it), **yaml-parse** (all 6 `config/*.yaml` parse), **unit-tests** (eval-receipt contract suite, 10 green). Folds framework's phased plan + scaffold's job table. Also: `jsonschema>=4.18` added to `requirements-dev.txt` (validator dep now reproducible in CI); dogfood receipts at `operations/receipts/hermes-006-ci-gate.eval_receipt.json` and `operations/receipts/hermes-006-phase3-receipt-required.eval_receipt.json`. **Phase-3 enforcement landed 2026-06-16 (`receipt-required` job).** | Stop if it conflicts with skill-governance CI — **did NOT fire** (skill-governance is `.02_Plugins/**`-scoped; no path/job/concurrency overlap) |
...
- [x] A single Hermes CI gate validates eval receipts and rejects invalid ones (HERMES-006: `hermes-gate.yml`, 4 jobs). _Phase-3 enforcement landed 2026-06-16: `receipt-required` fails a mission PR that ships no receipt (`no-receipt` label waives it for non-mission infra)._
