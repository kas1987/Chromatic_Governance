...
r = subprocess.run(
    ["python", ".agents/hooks/pre_tool_guard.py"],
...
r = subprocess.run(
    ["python", ".agents/hooks/post_tool_audit.py"],
    input=json.dumps({"tool_name": "Bash", "tool_input": {"command": "git status"}, "session_id": "t"}),
    text=True, capture_output=True,
)
...
r = subprocess.run(
    ["python", ".agents/hooks/subagent_stop.py"],
...
r = subprocess.run(
    ["python", ".agents/hooks/context_snapshot.py"],
...
r = subprocess.run(["python", ".github/scripts/governance_check.py"], text=True, capture_output=True)
